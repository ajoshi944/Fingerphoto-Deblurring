...
class BlurImage(object):
    ...
    def __init__(self, image_path, PSFs=None, part=None, path__to_save=None):
        ...
        self.original = np.array(image_path)
        if len(self.original.shape) == 2:
            self.original = np.stack((self.original,) * 3, 2)
        self.shape = self.original.shape
        if len(self.shape) < 3:
            raise Exception('We support only RGB images yet.')
        elif self.shape[0] != self.shape[1]:
...
"""if __name__ == '__main__':
    folder = '/home/n-lab/Amol/data/contactless/cropped/3/B/test'
    folder_to_save = '/home/n-lab/Amol/data/contactless/cropped/3/B/test_motion_blured'
    #params = [0.01, 0.009, 0.008, 0.007, 0.005, 0.003]
    params = [0.0005]
    for path in os.listdir(folder):
        print(path)
        trajectory = Trajectory(canvas=64, max_len=60, expl=np.random.choice(params)).fit(show=True)
        psf = PSF(canvas=64, trajectory=trajectory).fit(show=True)
